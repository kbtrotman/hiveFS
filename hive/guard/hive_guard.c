...
/* MariaDB helpers for chunk existence and blob storage (replacing RocksDB) */
static int mdb_exists_chunk(MDB* m, const char* hash){
  char sql[512];
  snprintf(sql, sizeof(sql), "SELECT 1 FROM chunks WHERE hash='%s' LIMIT 1", hash);
  if(mysql_query(m->conn, sql)!=0){
    LOGE("mysql_query failed: %s", mysql_error(m->conn));
    return -1;
  }
...
static int mdb_put_chunk(MDB* m, const char* hash, const unsigned char* data, size_t len){
  MYSQL_STMT* stmt = mysql_stmt_init(m->conn);
  if(!stmt) return -1;
  const char* sql = "INSERT INTO chunks_data(hash,data) VALUES(?,?) "
                    "ON DUPLICATE KEY UPDATE data=VALUES(data)";
  if(mysql_stmt_prepare(stmt, sql, (unsigned long)strlen(sql)) != 0){
    LOGE("mysql_stmt_prepare failed: %s", mysql_error(m->conn));
  ...
  bind[0].buffer = (void*)hash;
  bind[0].buffer_length = (unsigned long)strlen(hash);

  // data as blob
  bind[1].buffer_type = MYSQL_TYPE_BLOB;
  bind[1].buffer = (void*)data;
  bind[1].buffer_length = (unsigned long)len;

  if(mysql_stmt_bind_param(stmt, bind) != 0){
    LOGE("mysql_stmt_bind_param failed: %s", mysql_error(m->conn));
    mysql_stmt_close(stmt);
    return -1;
  }
  if(mysql_stmt_execute(stmt) != 0){
    LOGE("mysql_stmt_execute failed: %s", mysql_error(m->conn));
    mysql_stmt_close(stmt);
    return -1;
  }
  mysql_stmt_close(stmt);
...
static int handle_one(Client* c, ServerState* S){
  ...
  if(strcmp(type,"hello")==0){
    ...
    char* rep = jbuild_obj("type:s,mariadb:s","db_status", m?"ok":"down");
    ...
    if(!h){ char* e=mk_error("bad_request","missing hash"); write_framed_json(c->fd,e); free(e);}
      ...
      int ex = mdb_exists_chunk(&S->mdb, h);
      if(ex<0){ char* e=mk_error("db_error","blob error"); write_framed_json(c->fd,e); free(e); }
      else {
    ...
    if(!h || !b64 || !obj || !txn){
      ...
      if(!dedupe_hit){
        ...
        if(mdb_put_chunk(&S->mdb, h, raw, rlen)!=0){
          char* e=mk_error("db_error","mariadb blob insert failed"); write_framed_json(c->fd,e); free(e);
          goto store_done;
        }
      }
